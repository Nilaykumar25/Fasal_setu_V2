...
from compliance.guardrail import check_and_gate, check_input, ComplianceViolation
...
def run_query(user_input: str, context: dict = None, session_id: str = "default") -> dict:
    # ── Pre-execution input screening ─────────────────────────────────────────
    try:
        check_input(user_input, session_id=session_id)
    except ComplianceViolation as e:
        logger.warning("INPUT BLOCKED | session=%s | %s", session_id, e)
        return {
            "answer":  "That query involves a banned substance. Safe alternatives: neem oil 3mL/L, spinosad, or Bacillus thuringiensis. KVK helpline: 1800-180-1551.",
            "blocked": True,
            "phase":   "pre_execution",
            "reason":  str(e),
        }
    ...
    try:
        ...
        return check_and_gate("orchestrator", response, session_id=session_id)
